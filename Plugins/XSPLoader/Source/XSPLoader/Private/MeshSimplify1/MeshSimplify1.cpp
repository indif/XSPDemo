...
{
	const float Epsilon = THRESH_POINTS_ARE_SAME;
...
FOverlappingCorners::FOverlappingCorners(const std::vector<FVector3f>& InVertices, const std::vector<uint32_t>& InIndices)
{
	...
	for (int32_t i = 0; i < VertIndexAndZ.size(); i++)
	{
		...
		{
			if (std::abs(VertIndexAndZ[j].Z - VertIndexAndZ[i].Z) > THRESH_POINTS_ARE_SAME)
...
bool SimplyMesh(const std::vector<float>& InPositions, float PercentTriangles, float PercentVertices, std::vector<float>& OutPositions, std::vector<float>& OutNormals, std::vector<uint32_t>& OutIndices)
{
	std::vector<FVector3f> Positions;
	std::vector<FVector3f> SimpPositions, SimpNormals;
	...
	bool bSuccess = SimplyMesh(Positions, PercentTriangles, PercentVertices, SimpPositions, SimpNormals, OutIndices);
	if (!bSuccess)
		return false;

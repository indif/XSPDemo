...
// Kahan's algorithm
	...
	template< typename T >
	TVec3<T> operator*(T Scalar, const TVec3<T>& v)
	{
	...
	inline void FQuadric::Add(const FEdgeQuadric& __restrict EdgeQuadric, const FVector3f& Point)
	{
	...
	public:
		...
		void		Rebase(const FVector3f& Point, const float* Attributes, const float* AttributeWeights, uint32_t NumAttributes);
		void		Add(const FQuadricAttr& q, const FVector3f& Point, const float* Attribute, const float* AttributeWeights, uint32_t NumAttributes);
		void		Add(const FQuadricAttr& q, uint32_t NumAttributes);

		void		Zero(uint32_t NumAttributes);

		// Evaluate error at point with attributes and weights
		float		Evaluate(const FVector3f& Point, const float* Attributes, const float* AttributeWeights, uint32_t NumAttributes) const;

		// Calculate attributes for point and evaluate error
		float		CalcAttributesAndEvaluate(const FVector3f& Point, float* Attributes, const float* AttributeWeights, uint32_t NumAttributes) const;
	...
	template< uint32_t NumAttributes >
	...
	template< uint32_t NumAttributes >
	inline TQuadricAttr< NumAttributes >& TQuadricAttr< NumAttributes >::operator+=(const TQuadricAttr< NumAttributes >& __restrict q)
	{
		nxx += q.nxx;
		nyy += q.nyy;
		nzz += q.nzz;
	...
	template< uint32_t NumAttributes >
	inline float TQuadricAttr< NumAttributes >::CalcAttributesAndEvaluate(const FVector3f& Point, float* __restrict Attributes, const float* __restrict AttributeWeights) const
	{
	...
	public:
		...
		void		AddQuadric(const FQuadricAttr& q, uint32_t NumAttributes);
	...
	inline void FQuadricAttrOptimizer::AddQuadric(const FQuadric& __restrict q)
	{
		nxx += q.nxx;
		nyy += q.nyy;
		nzz += q.nzz;
	...
	inline void FQuadricAttrOptimizer::AddQuadric(const FQuadricAttr& __restrict q, uint32_t NumAttributes)
	{
